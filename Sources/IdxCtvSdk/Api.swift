...
final class Api {
    private static func makeRequest (
        ...
        guard var urlComponent = URLComponents(string: url) else {
            throw ECTVError.cannotCreateUrl(from: requestUrl)
        }
    ...
    private static func sendRequest(
        ...
        URLSession.shared.dataTask(with: request) {(data, response, error) in
            ...
            func handleResponse(_ error: ECTVError) {
                if (retryCount > 0) {
                    return sendRequest(
                        request: request,
            ...
            guard let data = data else {
                return handleResponse(ECTVError.responseIsEmpty)
            }
            ...
            if (httpResponse.statusCode != 200) {
                return handleResponse(ECTVError.requestError)
            }
